################################################
###        Sim C: Incorrect index groupings  ###
################################################
## May 11, 2021
## Run simulation C on subsample of NHANES data 
...
if(runLOCAL==TRUE){
  ...
  suffix <- paste0("_simC","_n",n,"_sd0",sub("\\.","",sd),"_iter") #paste0("_analysis","_n",n) ## output file doesnt have a suffix
  ...
  suffix <- paste0("_simC","_n",n,"_sd0",sub("\\.","",sd),"_iter",iter_no) ## append output file names with the iteration number, to be combined later
...
# exposure reponse
#### EDITED IN simC:
##### Swapped x7/8 with x17/18 here:
wx1 <- scale(cbind(x1[,1:6],x3[,7:8])%*%w1);mn1 <- mean(cbind(x1[,1:6],x3[,7:8])%*%w1);sd1 <- sd(cbind(x1[,1:6],x3[,7:8])%*%w1);med1 <- c(apply(cbind(x1[,1:6],x3[,7:8]),2,median)%*%w1-mn1)/sd1
wx2 <- scale(x2%*%w2);                      mn2 <- mean(x2%*%w2);                      sd2 <- sd(x2%*%w2);                      med2 <- c(apply(x2,2,median)%*%w2-mn2)/sd2
wx3 <- scale(cbind(x3[,1:6],x1[,7:8])%*%w3);mn3 <- mean(cbind(x3[,1:6],x1[,7:8])%*%w3);sd3 <- sd(cbind(x3[,1:6],x1[,7:8])%*%w3);med3 <- c(apply(cbind(x3[,1:6],x1[,7:8]),2,median)%*%w3-mn3)/sd3
...
### EDITED FOR SimC
h25 <- hfun((cbind(dat$bsmim$X25[[1]][,1:6],dat$bsmim$X25[[3]][,7:8])%*%w1-mn1)/sd1,(dat$bsmim$X25[[2]]%*%w2-mn2)/sd2,(cbind(dat$bsmim$X25[[3]][,1:6],dat$bsmim$X25[[1]][,7:8])%*%w3-mn3)/sd3)
h50 <- hfun((cbind(dat$bsmim$X50[[1]][,1:6],dat$bsmim$X50[[3]][,7:8])%*%w1-mn1)/sd1,(dat$bsmim$X50[[2]]%*%w2-mn2)/sd2,(cbind(dat$bsmim$X50[[3]][,1:6],dat$bsmim$X50[[1]][,7:8])%*%w3-mn3)/sd3)
h75 <- hfun((cbind(dat$bsmim$X75[[1]][,1:6],dat$bsmim$X75[[3]][,7:8])%*%w1-mn1)/sd1,(dat$bsmim$X75[[2]]%*%w2-mn2)/sd2,(cbind(dat$bsmim$X75[[3]][,1:6],dat$bsmim$X75[[1]][,7:8])%*%w3-mn3)/sd3)
