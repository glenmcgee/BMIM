...
################################################
## May 11, 2020
## Run simulation D on subsample of NHANES data 
...
if(runLOCAL==TRUE){
  ...
  suffix <- paste0("_simD","_n",n,"_sd0",sub("\\.","",sd),"_iter") #paste0("_analysis","_n",n) ## output file doesnt have a suffix
  ...
  suffix <- paste0("_simD","_n",n,"_sd0",sub("\\.","",sd),"_iter",iter_no) ## append output file names with the iteration number, to be combined later
...
mn1 <- apply(x1,2,mean); sd1 <- apply(x1,2,sd); med1 <- apply(x1,2,median)
mn2 <- apply(x2,2,mean); sd2 <- apply(x2,2,sd); med2 <- apply(x2,2,median)
mn3 <- apply(x3,2,mean); sd3 <- apply(x3,2,sd); med3 <- apply(x3,2,median)
...
#### EDITED IN simD:
##### Full BKMR--no index structure
hfun <- function(z1,z2,z3){
  hh <- 0
  for(jj in 1:ncol(z1)){
    hh <- hh + w1[jj]*h1fun(z1[,jj])
  }
  hh <- hh + w3[1]*z3[,1]+w3[2]*z3[,2]-w3[3]*z3[,3]^2+w3[4]*2*z3[,1]*z3[,2]
  return(0.25*hh)
}
h <- hfun(scale(x1),scale(x2),scale(x3)) ## edited to take matrices of exposures as inputs, rather than indices as inputs
...
h25 <- hfun((dat$bsmim$X25[[1]]-matrix(mn1,ncol=length(mn1),nrow=nrow(dat$bsmim$X25[[1]]),byrow=T))/matrix(sd1,ncol=length(sd1),nrow=nrow(dat$bsmim$X25[[1]]),byrow=T),(dat$bsmim$X25[[2]]-matrix(mn2,ncol=length(mn2),nrow=nrow(dat$bsmim$X25[[2]]),byrow=T))/matrix(sd2,ncol=length(sd2),nrow=nrow(dat$bsmim$X25[[2]]),byrow=T),(dat$bsmim$X25[[3]]-matrix(mn3,ncol=length(mn3),nrow=nrow(dat$bsmim$X25[[3]]),byrow=T))/matrix(sd3,ncol=length(sd3),nrow=nrow(dat$bsmim$X25[[3]]),byrow=T))
h50 <- hfun((dat$bsmim$X50[[1]]-matrix(mn1,ncol=length(mn1),nrow=nrow(dat$bsmim$X50[[1]]),byrow=T))/matrix(sd1,ncol=length(sd1),nrow=nrow(dat$bsmim$X50[[1]]),byrow=T),(dat$bsmim$X50[[2]]-matrix(mn2,ncol=length(mn2),nrow=nrow(dat$bsmim$X50[[2]]),byrow=T))/matrix(sd2,ncol=length(sd2),nrow=nrow(dat$bsmim$X50[[2]]),byrow=T),(dat$bsmim$X50[[3]]-matrix(mn3,ncol=length(mn3),nrow=nrow(dat$bsmim$X50[[3]]),byrow=T))/matrix(sd3,ncol=length(sd3),nrow=nrow(dat$bsmim$X50[[3]]),byrow=T))
h75 <- hfun((dat$bsmim$X75[[1]]-matrix(mn1,ncol=length(mn1),nrow=nrow(dat$bsmim$X75[[1]]),byrow=T))/matrix(sd1,ncol=length(sd1),nrow=nrow(dat$bsmim$X75[[1]]),byrow=T),(dat$bsmim$X75[[2]]-matrix(mn2,ncol=length(mn2),nrow=nrow(dat$bsmim$X75[[2]]),byrow=T))/matrix(sd2,ncol=length(sd2),nrow=nrow(dat$bsmim$X75[[2]]),byrow=T),(dat$bsmim$X75[[3]]-matrix(mn3,ncol=length(mn3),nrow=nrow(dat$bsmim$X75[[3]]),byrow=T))/matrix(sd3,ncol=length(sd3),nrow=nrow(dat$bsmim$X75[[3]]),byrow=T))
