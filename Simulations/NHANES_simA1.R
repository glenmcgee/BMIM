...
################################################
## Oct 28, 2020
## Run simulation A1 on subsample of NHANES data 
...
if(runLOCAL==TRUE){
  ...
  suffix <- paste0("_simA1","_n",n,"_sd0",sub("\\.","",sd),"_iter") #paste0("_analysis","_n",n) ## output file doesnt have a suffix
  ...
  suffix <- paste0("_simA1","_n",n,"_sd0",sub("\\.","",sd),"_iter",iter_no) ## append output file names with the iteration number, to be combined later
...
hfun <- function(z) 0.5+0.5*z ##1.5/(1+exp(-15*z)) -0.25*z #1.5/(1+exp(-15*z))
